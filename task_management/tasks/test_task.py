...
class TestTaskViews:
    ...
    def test_task_list_filter_by_status(self, create_authenticated_client, create_task):
        client, user = create_authenticated_client
        url = "/api/v1/tasks/?status=new"
        response = client.get(url)
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data["results"]) > 0
        assert response.data["results"][0]["status"] == "new"

    def test_task_list_filter_by_priority(
        self, create_authenticated_client, create_task
    ):
        client, user = create_authenticated_client
        url = "/api/v1/tasks/?priority=low"
        response = client.get(url)
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data["results"]) > 0
    ...
    def test_task_list_ordering(self, create_authenticated_client, create_task):
        ...
        assert (
            ...
            <= response.data["results"][-1]["priority"]
        )
